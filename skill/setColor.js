module.exports.setColor = async (interaction) => {
  ...
  switch (route) {
    case 1:
      guildRoles.find(role => role.name === memberId.slice(-8)).edit({
        color: colorCode
      });
    ...
    case 2:
      ...
      interaction.member.roles.add(guildRoles.find(role => role.name === memberId.slice(-8)));
      await interaction.reply({content: "(_゜∀゜)o彡ﾟもういっかい\n*(仕様です、断じてバグではありません byST)*", ephemeral: true});
      return;
  }
}