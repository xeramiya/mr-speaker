...
const client = new Client({
  intents: ["GUILDS", "GUILD_MEMBERS", "GUILD_EMOJIS_AND_STICKERS", "GUILD_MESSAGES", "GUILD_VOICE_STATES", "GUILD_PRESENCES", "GUILD_MESSAGE_REACTIONS"],
  partials: ['MESSAGE', 'REACTION']
});
const cron = require('node-cron');

// TOKENs
const TOKEN_MrSpeaker = process.env.TOKEN_MR_SPEAKER;
...
const slashCommands = [
  ...
  },
  {
    ...
    options: [{
        type: 'SUB_COMMAND',
        name: 'color',
        description: '[STEALTH]アンタの名前の文字色を変更するわよ!色コードを教えなさい!',
        options: [{
          type: 'STRING',
          name: 'colorcode',
  ...
  },
  {
    ...
    options: [{
      type: 'STRING',
      name: 'script',
      description: 'アタシに喋らせたいセリフを書くのよ。変なこと言ったら殺すからね。',
      required: true
    }]
  }
];
...
client.once('ready', async () => {
  console.log(">> System All Green!! <<\nNOW_RUNNING: Mr.Speaker_Ver" + mrSpeaker.version + "\n");

  client.user.setActivity(
    "Ver" + mrSpeaker.version,
    {
      type: 'PLAYING',
      url: 'https://uebit.tk/'
    }
...
client.on('messageReactionAdd', async reaction => {
  ...
  if (reaction.emoji.name == "dameda") {
    ...
    if (x < 0.5) {
      ...
      mrPlayer.mrPlayer(undefined, linkData.librarian('tekinosensuikanwohakken'), client);
    }
  }
});
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName === 'valve') {
    ...
    await mrPlayer.mrPlayer(interaction, 'https://cdn.glitch.com/3e3ea0c8-83cc-4a76-bb05-c9000eb70665%2Fvalve.ogg?v=1589352035171');
  }
  ...
  if (interaction.commandName === 'meet') {
    ...
    await mrPlayer.mrPlayer(interaction, linkData.librarian('emergencyMeeting'));
  }
...
client.on('messageCreate', async message => {
  ...
  // ボイスチャンネルテスト用
  if (message.content === "#11above") {
    if (message.member.voice.channel) {
      message.reply("AUDIO-TEST:START");
      await mrPlayer.mrPlayer(undefined, linkData.librarian('#11above'), client);
    }
  } else

  // ここすき
  if (message.content.match("ここすき|ここ好き") || message.content.match(":kokosuki:")) {
    await mrPlayer.mrPlayer(undefined, linkData.librarian('kokosuki'), client);
  } else

  // わかるマン
  if (message.content.match("わかるマン|わかるまん|ワカルマン|わかる|ワカル|wakaru|WAKARU|ﾜｶﾙ|わっかる|ワッカル") || message.content.match(":wakaruman:")) {
    await mrPlayer.mrPlayer(undefined, linkData.librarian('wakaruman'), client);
  } else

  // エヘッってなんだよ!
  if (message.content.match("えへっ|エヘッ|ｴﾍｯ|衛兵|エセ|衛星") || message.content.match(":ehettenandayo:")) {
    await mrPlayer.mrPlayer(undefined, linkData.librarian('ehettenandayo'), client);
  } else

  // 師匠スタンプボイス
  if (message.content.match(":hamburg:") || message.content.match("ハンバーグ")) {
    await mrPlayer.mrPlayer(undefined, linkData.librarian('hamburg'), client);
  } else
  
  // 呪語検知
  if (message.content.match("キーボード")) {
    await mrPlayer.mrPlayer(undefined, linkData.librarian('kbc'), client);
  } else

  // みなさ〜ん
  if (message.content.match("永田")) {
    await mrPlayer.mrPlayer(undefined, linkData.librarian('killYou'), client);
  } else

  // Fire Mario
  if (message.content.match("ファイア|マリオ|mario|Mario|MARIO")) {
    await mrPlayer.mrPlayer(undefined, linkData.librarian('fireMario'), client);
  } else

  // 恭平柴田の走り方
  if (message.content.match("柴田恭兵|恭平柴田|走|run|RUN|ハリウッド|HOLLYWOOD")) {
    await mrPlayer.mrPlayer(undefined, linkData.librarian('kyouheiShibata'), client);
  } else
  
  // ApertureScience
  if (message.content === "scienceIsFun") {
  ...
  // バーチャルデビルマン
  if (message.channel.id == kikisenId) {
    deviloid.deviloid(message, client);
  } else

  // 悪は裁く...
  if (message.member.roles.cache.has('810483288224563202')) {
